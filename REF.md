...
follow the pattern established here with `tripRoutes` and `userRoutes`. If
...
`app/models/trip.js`. Route files are somewhat similar to controllers in
...
curl --include --request POST http://localhost:8000/sign-up \
  ...
  --data '{
    "credentials": {
      "email": "an@trip.email",
      "password": "a trip password",
      "password_confirmation": "a trip password"
    }
  }'
```
